...
function keyPlay() {
    if (keydown) document.addEventListener("keydown", (e)=>{
        switch(e.key){
            case "ArrowLeft":
                game.moveLeft();
                gameBoardUpdate();
...
class Game {
    constructor(initialState){
        ...
        this.tableStart = this.gameStatus.map((row)=>[
                ...row
            ]);
    }
    moveLeft() {
        if (this.statusGame !== "playing") return;
        let change = false;
        for(let i = 0; i < this.gameStatus.length; i++){
            const row = this.gameStatus[i];
            let newRow = row.filter((n)=>n !== 0);
            newRow = this.cellGood(newRow);
            for(let n = 0; n < newRow.length; n++)if (this.gameStatus[i][n] !== newRow[n]) {
    ...
    moveRight() {
        ...
        for(let i = 0; i < this.gameStatus.length; i++){
            ...
            newRow = this.cellGood(newRow);
            newRow.reverse();
            for(let n = 0; n < row.length; n++)if (this.gameStatus[i][n] !== newRow[n]) {
                this.gameStatus[i] = newRow;
                change = true;
            }
        }
    ...
    moveUp() {
        ...
        for(let i = 0; i < this.gameStatus.length; i++){
            ...
            for(let k = 0; k < column.length; k++)if (this.gameStatus[k][i] !== newColumn[k]) {
                change = true;
    ...
    restart() {
        this.gameStatus = this.tableStart.map((row)=>[
                ...row
            ]);
        this.statusGame = "idle";
        this.gameScore = 0;
    ...
    randomCells(cell = 1) {
        for(let i = 0; i < cell; i++){
            ...
            if (this.gameStatus[rowIndex][cellsIndex] === 0) this.gameStatus[rowIndex][cellsIndex] = randomNum;
            else this.randomCells(1);
        }
    }
    cellGood(arr) {
        let newArr = [
        ...
        for(let j = 0; j < newArr.length; j++)if (newArr[j] === newArr[j + 1]) {
            newArr[j] *= 2;
    ...
    winGame() {
        if (this.gameStatus.flat().some((n)=>n === 2048)) this.statusGame = "win";
    }
    loseGame() {
        for(let i = 0; i < this.gameStatus.length; i++){
            const row = this.gameStatus[i];
            const column = this.gameStatus.map((n)=>n[i]);
            if (column.includes(0) || row.includes(0)) return;
            for(let j = 0; j < row.length; j++){
                if (row[j] === row[j + 1]) return;
            }
            for(let j = 0; j < row.length; j++){
                if (column[j] === column[j + 1]) return;
            }
        }
